...
class DownloadsService extends ChangeNotifier {
  ...
  List<DownloadMovie> get downloadedMovies => List.unmodifiable(_downloadedMovies);
  ...
  void startDownload(Movie movie) {
    final index = _downloadedMovies.indexWhere((element) => element.movie == movie);
  ...
  void _downloadMovie(DownloadMovie downloadMovie) {
    ...
    Timer.periodic(const Duration(milliseconds: 200), (timer) {
      ...
      final downloaded = min(downloadMovie.movie.fileSize, downloadMovie.downloadedSize + interval);
